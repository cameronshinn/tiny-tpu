...
Sends the read_inputs instruction to the TPU which sends the matrix over the bus
by sending one column at a time. The matrix is stored in the TPU's input memory
with the first element at address @addr.
...
address @addr into the systolic array to perform a matrix multiplication. Stores
the result in the accumulator table at row index @accum_row and column index
@accum_col.
...
@clear: Empty the accumulator table after reading (1 to empty, 0 to not empty)

Sends the store_outputs instruction to the TPU which reads the matrix from the
section of the accumulator table indexed by row @accum_row and column
...
Sends the write outputs instruction to the TPU which sends the matrix over the
bus by sending one row at a time. The matrix is read from the TPU's output
memory with the first row at address @addr.
